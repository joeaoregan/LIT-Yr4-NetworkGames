...
//#define CLI_PORT ntohs(client.sin_port)							
//void displayIPstuff(struct sockaddr_storage client, int sock, char cliName[INET6_ADDRSTRLEN], int port);
//char* displayIPstuff(struct sockaddr_storage client, int sock, char cliName[],  int port);
//char* displayIPstuff(struct sockaddr_storage client, int sock, int port);
//void displayIPstuff(struct sockaddr_storage client, int sock, int port);
...
int main (int argc, char * argv []) {							// Option to specify port as a command line parameter
	...
	//	cliName = displayNameAndPort(&client, cliName, sock, &port);		// CreateTCPSocket.h: Display the Client IP and Port number
		displayNameAndPort(&client, len, cliName, sizeof cliName, &sock, &port);		// CreateTCPSocket.h: Display the Client IP and Port number
...
socklen_t len = sizeof client;
	...
	if (client.ss_family == AF_INET) {
	    struct sockaddr_in *sock = (struct sockaddr_in *)&client;
	    port = ntohs(sock->sin_port);
	    inet_ntop(AF_INET, &sock->sin_addr, cliName, sizeof cliName);
	} else { // AF_INET6
	    struct sockaddr_in6 *sock = (struct sockaddr_in6 *)&client;
	    port = ntohs(sock->sin6_port);
	    inet_ntop(AF_INET6, &sock->sin6_addr, cliName, sizeof cliName);
	}
		printf("Handling client %s/%d\n", cliName, port);
*/
